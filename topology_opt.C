// topology_opt.C — optimized generation with progress & phase 4
// build (Linux, GCC):
//   g++ -std=c++17 -O3 -march=native -DNDEBUG -fopenmp -I/usr/include/eigen3 -I. topology_opt.C -o topology_opt
// build (macOS, Apple clang):
//   brew install libomp
//   clang++ -std=c++17 -O3 -march=native -DNDEBUG -Xpreprocessor -fopenmp \
//     -I/opt/homebrew/include/eigen3 -I. \
//     topology_opt.C -L/opt/homebrew/opt/libomp/lib -lomp -o topology_opt
...
// wrap connect to avoid throwing to outer loop
...
// common evaluation: IF -> b0Q -> (Time/Null) -> ForcedBlowdown -> serialize
static inline bool evaluate_and_append(const Eigen::MatrixXi &glued,
    ...
    const Eigen::MatrixXi &IF = g.GetIntersectionForm();
    const int R = IF.rows(), C = IF.cols();
    for (int r = 0; r < R; ++r) {
        for (int c = 0; c < C; ++c) {
            if (c) out_buf.push_back(' ');
            out_buf.append(std::to_string(IF(r, c)));
        }
        out_buf.push_back('\n');
    }
    out_buf.push_back('\n');
...
static inline void print_progress(const char* phase, long long current, long long total) {
    if (total == 0) return;
    const double pct = 100.0 * double(current) / double(total);
    std::cout << "[" << phase << "] progress: " << current << "/" << total
              << " (" << pct << "%)\n";
}

int main() {
    // force line-buffered stdout so progress shows immediately
    ...
    // -----------------------------
    {
        ...
        #pragma omp parallel
        {
            ...
            for (int i  = 0;  i < Nparams; ++i) {
			    ...
			    int aux;
			    if ( k < Ninsts-1) { aux = k; }
			    else if( k >=Ninsts-1) {aux = Ninsts-1;}
                        for (int l = 0; l <= aux; ++l) {
                            TheoryGraph G;
                            auto S1   = G.add(s(params[i]));
