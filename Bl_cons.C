        ...
        const std::vector<Eigen::MatrixXi>& seq) // 단계별 Intersection Form
{
    try {
        /* 1) 경로 구성 -------------------------------------------------- */
        fs::path base_dir = "Bl_AF_SUGRA_" + std::to_string(i);
...
int main () 
{
	...
	fs::path out_dir = std::string("./Bl_AF_SUGRA_") + std::to_string(base);
	...
	for (int k =1; k < test->GetT(); k++)
	{
		test->intersect(k,k+1);
	}

	test->AddT(-1);
	test->intersect(test->GetT(),23);
	test->SetElement(test->GetT()-4,test->GetT()-4,-5);
